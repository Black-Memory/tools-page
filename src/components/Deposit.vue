<template>
  <div class="container">
    ...
    <v-form style="width: 100%;">
      ...
      <v-text-field v-model="formData.privateKey" :append-icon="show1 ? 'mdi-eye' : 'mdi-eye-off'" label="私钥"
      ...
      <v-btn :color="running ? 'red' : 'light-blue'" size="x-large" @click="submitForm">{{ running ? "关闭" : "启动" }}
      </v-btn>
    <v-input v-model="formData.fastGasPrice" label="Fast Gas Price" type="number" />

      <v-btn color="light-blue" size="x-large" @click="quicken" style="margin-left:40px">加速</v-btn>
    </v-form>
    <v-textarea class="log-panel" v-model="logs" label="日志" readonly auto-grow :value="logsText" variant="outlined" />
